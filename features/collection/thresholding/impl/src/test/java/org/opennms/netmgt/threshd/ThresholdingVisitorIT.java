...
public class ThresholdingVisitorIT {
    ...
    *//**
        * Similar to {@link #testThresholdsFiltersOnNodeResource()}, but we generate the collection set using the CollectionSetBuilder instead of using SnmpCollector specific
        * types.
        */
    /*
    @Test
    ...
    *//**
        * Similar to {@link #testThresholdFiltersOnGenericResource()}, but we generate the collection set using the CollectionSetBuilder instead of using SnmpCollector specific
        * types.
        */
    /*
    @Test
    ...
    *//**
        * Verifies that we are able to generate thresholds for CollectionSets using GenericTypeResource resources that use the SiblingColumnStorageStrategy. Resources of this type
        * are commonly used by the WS-Man collector, where data is returned in a tabular format and the resource name is set using the SiblingColumnStorageStrategy. In the case of
        * the WS-Man collector, the instance ids are generated systematically and will differ from one collection run to another. For this reason, it's important that the
        * thresholder keys it's state based off of the resource label, and not the instance id - this test will validate this.
        *//*
          @Test
